#!/usr/bin/env python3
"""Load entry metadata and embeddings into Postgres with pgvector."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Load Zetesis entry documents and embeddings into a pgvector-enabled Postgres database.",
    )
    parser.add_argument(
        "--entries",
        type=Path,
        default=Path("sep_entries.json"),
        help="Path to sep_entries.json.",
    )
    parser.add_argument(
        "--embeddings",
        type=Path,
        default=Path("sep_entry_embeddings.npy"),
        help="Path to sep_entry_embeddings.npy.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        "--truncate",
        action="store_true",
        help="Truncate the entries table before loading.",
    )
    parser.add_argument(
        "--drop-index-first",
        action="store_true",
...
def validate_inputs(entries: list[dict[str, object]], embeddings: np.ndarray) -> None:
    if not entries:
        raise ValueError("No entry records found.")
    ...
    if len(entries) != embeddings.shape[0]:
        raise ValueError(
            f"Entry count ({len(entries)}) does not match embedding rows ({embeddings.shape[0]})."
        )
    ...
    for index, entry in enumerate(entries):
        ...
        if missing:
            ...
            raise ValueError(f"Entry at index {index} is missing required fields: {missing_list}")
...
def entry_rows(
    ...
    for index in range(start, end):
        ...
        rows.append(
            (
                uuid.UUID(str(entry["entry_id"])),
                fix_mojibake(str(entry["entry_slug"])),
                fix_mojibake(str(entry["entry_title"])),
                str(entry["url"]),
                int(entry["section_count"]),
                fix_mojibake(str(entry["section_outline"])),
                fix_mojibake(str(entry["intro_text"])),
                fix_mojibake(str(entry["search_text"])),
...
def main() -> None:
    ...
    entries = load_entries(args.entries, args.limit)
    embeddings = load_embeddings(args.embeddings, args.limit)
    validate_inputs(entries, embeddings)

    total_rows = len(entries)
    print(f"Validated {total_rows} rows from {args.entries} and {args.embeddings}")
    ...
    with psycopg.connect(database_url) as conn:
        ...
        with conn.cursor() as cur:
            ...
            if args.truncate:
                cur.execute("TRUNCATE TABLE entries;")
                print("Truncated entries table.")
            ...
            for start in range(0, total_rows, args.batch_size):
                ...
                rows = entry_rows(entries, embeddings, start, end)
            ...
            print(f"Finished loading {total_rows} rows into entries.")
